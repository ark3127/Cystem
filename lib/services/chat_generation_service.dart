...
class ChatGenerationService {
  ...
  Future<GeminiImageGenerationResult> editImage({required ChatAttachment source, required String instruction}) =>
      _imageGenerationService.editImage(source: source, instruction: instruction);
  ...
  Stream<ChatStreamEvent> generate(
    ...
    for (final message in messages) {
      ...
      if (message.isUser && message.attachments.isNotEmpty) {
        ...
        token.throwIfCancelled();
        prepared.add(message.copyWith(
          content: _withVisionContext(message.content, vision),
          attachments: const [],
        ));
      } else {
    ...
    if (wantsWebImages) {
      ...
      for (final image in images) {
        token.throwIfCancelled();
        await onGeneratedImage?.call(image, null);
      }
      return;
    }
    ...
    while (true) {
      ...
      await for (final event in _apiService.streamMessage(
        _messagesForNemotron(prepared, imageRequest: wantsImage),
        tools: (wantsImage || wantsWebSearch) ? const [] : _toolRegistry.tools,
        cancellationToken: token,
      )) {
        token.throwIfCancelled();
        events.add(event);
        // Image generation is an image-only operation: never expose Nemo's private prompt.
        if (!wantsImage) {
      ...
      prepared.add(ChatMessage(
        id: '${DateTime.now().microsecondsSinceEpoch}_assistant_tool',
      ...
      for (final call in calls) {
        ...
        prepared.add(ChatMessage(
          id: '${DateTime.now().microsecondsSinceEpoch}_tool_${call.id}',
          content: result,
  ...
  String _latestUserText(List<ChatMessage> messages) {
    for (var i = messages.length - 1; i >= 0; i--) if (messages[i].isUser) return messages[i].content.trim();
    return '';
  }
  ...
  String _withVisionContext(String userText, String analysis) => '${userText.trim().isEmpty ? '(No text was provided with the image.)' : userText.trim()}\n\n[PRIVATE GEMINI VISION CONTEXT]\n$analysis\n[END PRIVATE GEMINI VISION CONTEXT]';

  List<ChatToolCall> _completedToolCalls(List<ChatStreamEvent> events) {
    final calls = <String, ChatToolCall>{};
    for (final event in events) for (final call in event.toolCalls) calls[call.id] = call;
  ...
  ChatApiMetadata? _metadata(List<ChatStreamEvent> events) {
    for (final event in events.reversed) {
      if (event.responseId != null || event.model != null || event.finishReason != null || event.hasUsage) {
        return ChatApiMetadata(responseId: event.responseId, model: event.model, finishReason: event.finishReason, promptTokens: event.promptTokens, completionTokens: event.completionTokens, totalTokens: event.totalTokens);
      }
    }
    return null;
  }
