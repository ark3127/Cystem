...
class ChatGenerationService {
  ...
  Stream<ChatStreamEvent> generate(
    ...
    try {
      ...
      while (true) {
        ...
        await for (final event in _apiService.streamMessage(apiMessages, tools: wantsImage ? const [] : _toolRegistry.tools, cancellationToken: token)) {
          token.throwIfCancelled();
          events.add(event);
          yield event;
        }
        ...
        final assistantMessage = ChatMessage(id: '${DateTime.now().microsecondsSinceEpoch}_tool_assistant', content: _assistantText(events), role: MessageRole.assistant, createdAt: DateTime.now(), reasoningContent: _assistantReasoning(events), toolCalls: toolCalls, apiMetadata: _metadata(events));
        ...
        for (final call in toolCalls) {
          ...
          if (onToolMessage != null) {
            await onToolMessage(ChatMessage(id: '${DateTime.now().microsecondsSinceEpoch}_tool_status_${call.id}', content: call.name == 'web_search' ? '⏳ Searching the web…' : '⏳ Running ${call.name}…', role: MessageRole.tool, createdAt: DateTime.now(), toolCallId: call.id, toolName: call.name));
          }
          String result;
          try {
            ...
            token.throwIfCancelled();
            result = call.name == 'web_search' ? 'Web search failed: $error\n\nPlease try the search again with a different query.' : 'Tool execution failed: $error';
          }
          token.throwIfCancelled();
          final toolMessage = ChatMessage(id: '${DateTime.now().microsecondsSinceEpoch}_tool_result_${call.id}', content: result, role: MessageRole.tool, createdAt: DateTime.now(), toolCallId: call.id, toolName: call.name);
  ...
  bool _latestUserRequestsImage(List<ChatMessage> messages) {
    for (var i = messages.length - 1; i >= 0; i--) {
      ...
      return RegExp(r'\b(generate|create|draw|make|render|design|produce|paint|illustrate)\b[\s\S]{0,80}\b(image|picture|photo|illustration|artwork|wallpaper|logo|poster|diagram)\b|\b(image|picture|photo|illustration|artwork|wallpaper|logo|poster|diagram)\b[\s\S]{0,40}\b(generate|create|draw|make|render|design|produce)\b', caseSensitive: false).hasMatch(message.content);
    }
    return false;
  }
