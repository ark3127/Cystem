...
/// Internal signal used to let [RetryService] distinguish a transient
/// failure (worth retrying) from a permanent one (fail fast).
class _SearchAttemptFailure implements Exception {
  _SearchAttemptFailure({
    required this.code,
...
class WebSearchService implements ChatToolExecutor {
  ...
  ///
  /// Transient failures (429/5xx/network/parse errors) are retried with
  /// backoff via [RetryService] before giving up. Configuration errors
  /// (missing API key) fail immediately since retrying can't fix them.
  Future<String> search(String query) async {
    ...
    try {
      return await _retry.run(
        () => _attemptSearch(query, apiKey),
        maxAttempts: 3,
        initialDelay: const Duration(seconds: 1),
        shouldRetry: (error) => error is _SearchAttemptFailure ? error.retryable : true,
      );
    } on _SearchAttemptFailure catch (error) {
      return _serviceError(error.code, error.retryable, error.details, httpStatus: error.httpStatus);
    } on FormatException {
  ...
  Future<String> _attemptSearch(String query, String apiKey) async {
...
$query''',
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        throw _SearchAttemptFailure(
          code: _classifyStatus(response.statusCode),
      ...
      if (text.isEmpty) {
        throw _SearchAttemptFailure(
          code: 'EMPTY_RESULT',
          retryable: true,
          details: 'Google Search returned no usable context.',
        );
      }
