...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _send() async {
    ...
    setState(() => _webSearchMode = false);
    await _save();
    _scrollToBottom(jump: true);
    await _generateResponse(forceWebSearch: useWebSearch);
  }

  Future<void> _generateResponse({bool forceWebSearch = false}) async {
    final chat = _chat;
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                const _Chip(icon: Icons.lightbulb_outline_rounded, label: 'Explore an idea'),
                const _Chip(icon: Icons.code_rounded, label: 'Write code'),
                _Chip(
                  icon: Icons.language_rounded,
                  label: 'Search the web',
                  onPressed: () {
                    setState(() => _webSearchMode = true);
                    _snack('Web search is on for your next message.');
                  },
                ),
                const _Chip(icon: Icons.image_outlined, label: 'Create an image'),
              ],
            ),
          ],
        ),
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: _generating
                    ? 'CYSTEM is thinking…'
                    : (_webSearchMode ? 'Search the web for…' : 'Message CYSTEM'),
                ...
                prefixIcon: Row(
                  ...
                  children: [
                    ...
                    IconButton(
                      tooltip: _webSearchMode ? 'Web search on for the next message' : 'Search the web',
                      onPressed: _generating ? null : () => setState(() => _webSearchMode = !_webSearchMode),
                      isSelected: _webSearchMode,
                      icon: const Icon(Icons.language_rounded),
                      selectedIcon: Icon(Icons.language_rounded, color: AppTheme.primary),
                    ),
                  ],
                ),
                suffixIcon: Padding(
...
class _Chip extends StatelessWidget {
  const _Chip({required this.icon, required this.label, this.onPressed});
  final IconData icon;
  final String label;
